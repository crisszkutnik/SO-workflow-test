...
int main() {
    ...
    /*job_queue = queue_create();					// Cola
    vacía

    pthread_mutex_init(&mutex_impresion, NULL);	// Mutex común y corriente
    sem_init(&sem_size_cola, 0, TAMAN_COLA);	// Semáforo inicializado en
    TAMAN_COLA (ver queue_synced_*) sem_init(&sem_cola_vacia, 0, 0);
    // Semáforo inicializado en 0 (ver queue_synced_*)
    ...
    // Terminación (espera a que todos los hilos terminen para recién después
    retornar main)
