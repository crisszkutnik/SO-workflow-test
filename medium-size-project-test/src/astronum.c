...
static int cadenaNumerica(char *cadena) {
  for (int out = 0; cadena[out] != '\0'; out++)
    if (!isdigit(cadena[out]))
      return 0;
  return 1;
}

// Crea un puntero a NumeroAstronomico desde cadena
// Considera los errores: cadena invalida, cadena vacía, cadena muy larga,
// errores de malloc Si la cadena dada solamente tiene "0"s el comportamiento es
// indefinido.
...
// Ejemplo de error: CrearDesdeCifraSeguidaDeCeros(1427, 98); -> 102 dígitos,
// error En todos los casos, la cantidad de ceros no puede superar los 99 En
// casos de error retorna NULL
NumeroAstronomico *CrearDesdeCifraSeguidaDeCeros(int num, int ceros) {
  ...
  for (int i = 0; i < ceros; i++)
    cadena[i + longitud_num] = '0'; // llena el resto con 0
...
// Si el programa se ejecuta más de una vez por segundo, el resultado generado
// es el mismo
...
int EsOverflow(NumeroAstronomico *a) { return a->longitudError == Overflow; }
...
int EsError(NumeroAstronomico *a) {
  return EsSecuenciaInvalida(a) || EsOverflow(a) || EsSecuenciaNula(a) ||
         EsPunteroNulo(a);
}
...
void Mostrar(NumeroAstronomico *a, int grupos) {
  ...
  case PunteroNulo:
    ...
    return;
  case CadenaInvalida:
    fprintf(stderr, "Cadena invalida\n\n");
    return;
  case CadenaNula:
    fprintf(stderr, "Cadena nula\n\n");
    return;
  }
  // end errores //
...
static int longmax(NumeroAstronomico *a, NumeroAstronomico *b) {
  return (a->longitudError > b->longitudError ? a->longitudError
                                              : b->longitudError);
}
...
static char *sumaCadenas(const char *a, int lena, const char *b, int lenb) {
  ...
  for (int i = 1; i < longmax; i++) {
    // Suma dígitos de ambas cadenas (la mas corta rellenada con ceros a la
    // izquierda)
    char padding_or_num =
        i < diff ? '0' : corta[i - diff]; // 0 o dígito (cadena corta)
    int cifra =
        can(padding_or_num) + can(larga[i - 1]); // dígito (cadena larga)
    ...
    carries = 1;
    while (buffer > 9) {
      buffer = can(salida[i - carries] +
                   1); // El dígito N (carries) lugares a la izquierda + 1
      salida[i - carries] =
          nac(buffer % 10); // Se escribe en salida el valor de la unidad de eso
  ...
  if (salida[0] == '0')
    *(salida)++; // Si empieza en 0 movemos el puntero 1 lugar para que no lo
                 // haga
...
// Si uno de los números es secuencia nula, se cuenta como cero (si son nulos
// los dos retorna nulo) Si ocurre un error de algún otro tipo, retorna NULL
NumeroAstronomico *Sumar(NumeroAstronomico *a, NumeroAstronomico *b) {
  ...
  if (!EsError(a) && !EsError(b)) { // todo en orden
    char *suma =
        sumaCadenas(a->entero, a->longitudError, b->entero, b->longitudError);
