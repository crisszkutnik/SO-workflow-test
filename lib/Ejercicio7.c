...
pthread_mutex_t mutex_impresion;	// Para que no hagan push o pop más de una PC a la vez
////////////////////////////////////////
...
void trabajar_PC(void* args) {
	// t_queue* job_queue = (t_queue*) args; -> parece que job_queue tiene que ser global
	//										   para que todos los hilos la puedan usar.
	//										   Antes se estaba pasando como parámetro a esta func
	char* nombre = (char*) args;			// Un mejor argumento!
	for (int i = 0; i < 10; i++) {
		mandar_a_imprimir(job_queue, nombre);
		usleep((unsigned int) 1);			// Ni idea por qué pero usan (unsigned int) para esto
	}
}
...
void* queue_synced_pop(t_queue* job_queue) {
	...
	sem_post(&sem_size_cola); // Señala que ahora hay 1 espacio más para hacer push

	return elem;
}

void queue_synced_push(t_queue* job_queue, t_print_job* job) {
	sem_wait(&sem_size_cola); // si está en 0, no tiene más espacio, impresora espera

	pthread_mutex_lock(&mutex_impresion);
	queue_push(job_queue, (void*) job);
	pthread_mutex_unlock(&mutex_impresion);

	sem_post(&sem_cola_vacia); // Le indica que ya se puede hacer pop, la cola no está más vacía
}
