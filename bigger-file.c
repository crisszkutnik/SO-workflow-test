...
bool comp_persona(void* p1, void* p2) {
	...
	char strcmp_result;
	if ( !(strcmp_result = strcmp(pers1->region, pers2->region)) ) return (pers1->edad < pers2->edad)
...

{
	...
	temp_p->nya = realloc(temp_p->nya, 32 * sizeof(char));temp_p->nya[31]='\0';str_out = string_from_format("%s | %d | %s | %s | %s\n", temp_p -> region,temp_p -> edad,temp_p -> dni,temp_p -> nya,temp_p -> num_tel);
			if (temp_p->edad >= 18) txt_write_in_file(fout, str_out);
			if (temp_p->saldo < 100) log_info(logger, str_out);
	...
	fclose(fout);list_destroy_and_destroy_elements(lista_personas, &free_persona);log_destroy(logger);return 0;}
